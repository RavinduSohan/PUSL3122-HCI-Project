...
export default function RegisterPage() {
  ...
  useEffect(() => { document.title = 'Create Account — FurniView'; }, []);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      toast.success('Account created! Welcome to FurniView.');
      ...
      <div className="flex-1 flex items-center justify-center px-8 py-12 bg-white overflow-y-auto">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Start designing beautiful rooms today</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text-muted)' }}>Account Type</label>
              <select
                {...register('role')}
                className="w-full px-4 py-2.5 border rounded-lg text-sm bg-white"
                style={{ borderColor: 'var(--border)' }}
              >
                <option value="user">User — design rooms independently</option>
                <option value="admin">Admin — furniture shop staff</option>
              </select>
            </div>
